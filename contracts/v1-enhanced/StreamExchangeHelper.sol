...
import {ISuperfluid, ISuperToken, ISuperToken, ISuperAgreement} from "@superfluid-finance/ethereum-contracts/contracts/interfaces/superfluid/ISuperfluid.sol";
...
library StreamExchangeHelper {
    ...
    function _emergencyDrain(StreamExchangeStorage.StreamExchange storage self)
        public
    {
        require(
        ...
        )
    {
        ...
        uint256 _time = self.oracle.getTimestampbyRequestIDandIndex(
            _requestId,
            _count - 1
        );
    ...
    function _distribute(
        ...
        uint256 outputBalance = ISuperToken(self.outputToken).balanceOf(
            address(this)
        );
        ...
        // Calculate subside
        uint256 subsidyAmount = (block.timestamp - self.lastDistributionAt) *
            self.subsidyRate;

        // Confirm the app has enough to distribute
        require(
            self.outputToken.balanceOf(address(this)) >= actualAmount,
            "!enough"
        );
        ...
        require(
            self.inputToken.balanceOf(address(this)) /
                10 **
                    (18 -
                        ERC20(self.inputToken.getUnderlyingToken())
                            .decimals()) ==
                0,
            "!sellAllInput"
        );
    ...
    function _initalizeLiquidityMining(
        StreamExchangeStorage.StreamExchange storage self
    ) internal {
